...
router.get('/productos/codigo/:codigoProducto', (req, res) => {
    const { codigoProducto } = req.params;
    console.log("GET : /productos/codigo/" + codigoProducto);
...
// Endpoint para simular la edición de un producto
router.put('/productos/:id', (req, res) => {
    const idProducto = req.params.id;
    const producto = req.body;
    console.log("PUT : /productos/:" + idProducto);

    console.log(`Editando producto con ID: ${idProducto}`);
    console.log("Datos del producto recibidos:", producto);

    // Simular una edición exitosa
    res.status(200).json({
...
// Actualizar stock de producto (compra - incrementar)
router.put('/productos/:id/actualizarStock', (req, res) => {
    const { id } = req.params;
    const { cantidad } = req.body;
    console.log(`PUT : /productos/${id}/actualizarStock`);

    const producto = productos.find(p => p.IdProducto == id);
    if (producto) {
        producto.Stock += cantidad;
        res.status(200).json({ message: "Stock actualizado correctamente", producto });
    } else {
        res.status(404).json({ message: "Producto no encontrado" });
    }
...
// Actualizar stock de producto (venta -decrementar)
router.put('/productos/:id/actualizarStockVenta', (req, res) => {
    const { id } = req.params;
    const { cantidad } = req.body;
    console.log(`PUT : /productos/${id}/actualizarStockVenta`);
...
router.get('/productos/masvendidos', (req, res) => {
    console.log("GET : /productos/masvendidos");
    ...
    // Simular ventas generando números aleatorios para TotalVendido
    let productosConVentas = productos.map(producto => {
        return {
            ...producto,
            TotalVendido: Math.floor(Math.random() * 100) // Simular un total vendido aleatorio
        };
    });

    // Filtrar productos dentro del rango de fechas
    let productosFiltrados = productosConVentas.filter(producto => {
        let fechaRegistro = new Date(producto.FechaRegistro);
        return fechaRegistro >= fechaDesde && fechaRegistro <= fechaHasta;
    });

    // Ordenar por TotalVendido en orden descendente y obtener los top N
    let productosMasVendidos = productosFiltrados.sort((a, b) => b.TotalVendido - a.TotalVendido).slice(0, topN);
